...
# -*- coding: utf-8 -*-
"""
Auron Search Bot - FINAL PRODUCTION VERSION
С полной диагностикой и обработкой ошибок
"""
...
async def cmd_start(m: Message):
    ...
    await m.answer(
        ...
        "Наш бот — это профессиональный инструмент для поиска свободных юзернеймов.\n\n"
...
async def cmd_find_username(m: Message):
    print(f"✅ find_username from {m.from_user.id}")
...
async def cmd_profile(m: Message):
    print(f"✅ profile from {m.from_user.id}")
    user = await get_user(m.from_user.id)
    limit = get_limit(user)
    searches = user.get("searches", 0)
...
async def cmd_support(m: Message):
    await m.answer(
        "🆘 *Поддержка*\n\nПо всем вопросам: @Auron_Search_Bot",
        parse_mode="Markdown",
        reply_markup=get_main_keyboard()
    )
...
async def cmd_my_usernames(m: Message):
    ...
    if not rows:
        await m.answer("📭 У вас нет сохранённых username.", reply_markup=get_main_keyboard())
        return
    await m.answer(
...
async def cb_choose_len(c: CallbackQuery):
    ...
    if length == 5:
        ...
        if not is_premium(user):
            return await c.answer("❌ Длина 5 доступна только Premium! Купите Premium в разделе 💎", show_alert=True)
...
async def cmd_admin_panel(m: Message):
    if m.from_user.id != config.admin_id:
        await m.answer("⛔ Доступ запрещён!", reply_markup=get_main_keyboard())
        return
    await m.answer(
        "🔧 *Админ панель*\n\nВыберите действие:",
        parse_mode="Markdown",
...
async def cmd_admin_payments(m: Message):
    if m.from_user.id != config.admin_id:
        return
...
async def cmd_premium_give(m: Message):
    ...
    args = m.text.split()
    if len(args) < 2:
        await m.answer("❌ /premium <user_id> [days] (-1=forever, 0=remove)", reply_markup=get_admin_keyboard())
        return
    try:
        uid = int(args[1])
        days = int(args[2]) if len(args) > 2 else 30
        await set_premium(uid, days)
        await m.answer(f"✅ Premium выдан {uid} на {'навсегда' if days == -1 else f'{days} дн.'}", reply_markup=get_admin_keyboard())
    except:
        await m.answer("❌ Ошибка", reply_markup=get_admin_keyboard())
...
async def cmd_ban(m: Message):
    ...
    args = m.text.split()
    if len(args) < 2:
        await m.answer("❌ /ban <user_id> [minutes]", reply_markup=get_admin_keyboard())
        return
    try:
        uid = int(args[1])
        minutes = int(args[2]) if len(args) > 2 else 5
        await app.rate_limiter.ban_user(uid, minutes * 60)
        await m.answer(f"✅ Пользователь {uid} забанен на {minutes} мин", reply_markup=get_admin_keyboard())
    except:
        await m.answer("❌ Ошибка", reply_markup=get_admin_keyboard())
...
async def handle_txid(m: Message, state: FSMContext):
    ...
    await m.answer("✅ Заявка отправлена! Администратор проверит.", reply_markup=get_main_keyboard())
    await state.clear()
...
async def payment_success(m: Message):
    ...
    await m.answer("✅ *Premium активирован!*", parse_mode="Markdown", reply_markup=get_main_keyboard())
...
def register_handlers(dp: Dispatcher):
    ...
    dp.message.register(cmd_admin_stats, F.text == "📊 Статистика")
    dp.message.register(cmd_admin_payments, F.text == "💰 Заявки USDT")
    dp.message.register(cmd_start, F.text == "🔙 В главное меню")
