...
def safe_handler(func):
    """Безопасный декоратор для всех хендлеров"""
    @wraps(func)
    ...
    async def wrapper(message: types.Message, *args, **kwargs):
        ...
        try:
            ...
            if member.status not in ("member", "administrator", "creator"):
                ...
                return  # важно: выйти, чтобы не выполнять основной код
        except Exception as e:
            logger.warning(f"Subscription check failed: {e}")
            # Не блокируем бота, если проверка недоступна
            return await func(message, *args, **kwargs)
        return await func(message, *args, **kwargs)
    return wrapper
...
@safe_handler
async def usdt_chosen(call: types.CallbackQuery, state: FSMContext):
    if not await callback_limiter.check(call.from_user.id):
        await call.answer("⚠️ Слишком много действий. Подождите.", show_alert=True)
        return
    uid = call.from_user.id
    parts = call.data.split("_")
    tariff = parts[1]  # "7d", "30d", "forever"
    price = float(parts[2])
    days = USDT_DAYS[tariff]
    # Сохраняем информацию о выбранном тарифе в FSM (чтобы потом подтвердить)
    await state.update_data(tariff=tariff, price=price, days=days)
    await safe_edit(call.message.chat.id, call.message.message_id,
                    f"💰 Оплата USDT\n\n"
                    f"Сумма: {price}$ USDT (TRC20)\n"
                    f"Кошелек: `{USDT_WALLET}`\n\n"
                    f"После перевода отправьте TXID (хеш транзакции) сообщением в этот чат.",
...
@safe_handler
async def usdt_txid(message: types.Message, state: FSMContext):
    txid = message.text.strip()
    if len(txid) < 30:
        await message.answer("❌ TXID похож на неверный. Попробуйте ещё раз.")
        return
    ...
    if not tariff or not price:
        await message.answer("❌ Сессия устарела. Выберите тариф заново.")
        await state.clear()
        return
    await db.create_payment(message.from_user.id, "usdt", tariff, price, days, txid)
    await message.answer(
        f"✅ TXID принят! Администратор проверит транзакцию и активирует Premium.\n"
        f"Ваш TXID: `{txid}`",
        parse_mode="Markdown"
    )
    # Уведомляем админа
...
@safe_handler
async def usdt_non_text(message: types.Message, state: FSMContext):
    await message.answer("❌ Отправьте TXID текстом.")
...
async def admin_panel(call: types.CallbackQuery):
    ...
    await call.answer("🔧 Админ-панель в разработке", show_alert=True)
...
if __name__ == "__main__":
    asyncio.run(main()) сюда же надо добавить токег
