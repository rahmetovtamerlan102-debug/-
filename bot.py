...
def set_premium(uid, days):
    with safe_db() as conn:
        ...
        # Обновляем и убеждаемся, что пользователь существует
        c.execute("UPDATE users SET premium_until=? WHERE user_id=?", (until, uid))
        if c.rowcount == 0:
            c.execute("INSERT INTO users (user_id, premium_until, last_date) VALUES (?, ?, ?)",
                      (uid, until, str(date.today())))
        logger.info(f"Premium установлен для {uid} до {until}")
...
# ---------- ПОДДЕРЖКА (без дублей) ----------
waiting_for_support = {}

@bot.message_handler(func=lambda m: m.text == "🆘 Поддержка")
@sub_required_msg
def support_cmd(m):
    # Если пользователь уже ожидает отправки – не создаём новое состояние
    if waiting_for_support.get(m.from_user.id):
        bot.send_message(m.chat.id, "Вы уже отправили сообщение в поддержку. Ожидайте ответа.")
        return
...
def forward_support(m):
    ...
    # Сбрасываем состояние строго после отправки
...
def pre_checkout(q):
    try:
        ...
        bot.answer_pre_checkout_query(q.id, ok=False, error_message="Ошибка платежа")
