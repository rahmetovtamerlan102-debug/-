...
# -*- coding: utf-8 -*-
"""
Auron Search Bot - RENDER READY (Polling Only)
Работает 24/7 на Render без webhook
"""
...
logger.info(f"   Mode: POLLING (Render)")
...
class RateLimiter:
    ...
    async def check(self, key: str, limit: int, window: int) -> Tuple[bool, int]:
        ...
        if len(self.requests[key]) > self.max_history:
            self.requests[key] = self.requests[key][-self.max_history:]
...
async def cmd_profile(m: Message):
    ...
    await m.answer(
        f"👤 *Личный кабинет*\n\n🔎 Использовано: {searches_text}\n🎫 Осталось: {remaining}\n👥 Приглашено друзей: {user.get('refs', 0)}\n💎 Premium: {prem_text}\n\n🔗 *Реферальная ссылка:*\n`{link}`\n\n📋 Сохранённые: /my_usernames",
        parse_mode="Markdown",
        reply_markup=get_main_keyboard()
    )
...
async def cb_generate(c: CallbackQuery):
    ...
    if limit == float("inf"):
        ...
        remaining = f"\n\n🎫 Осталось попыток: {max(0, int(limit) - new_searches)}"
...
async def cb_stars_pay(c: CallbackQuery):
    ...
    try:
        ...
        await c.answer("⚠️ Оплата через Stars временно недоступна. Используйте USDT.", show_alert=True)
    await c.answer()
...
async def handle_txid(m: Message, state: FSMContext):
    ...
    await m.answer("✅ Заявка отправлена! Администратор проверит её.", reply_markup=get_main_keyboard())
    await state.clear()
...
async def cmd_ban(m: Message):
    ...
    try:
        ...
        await m.answer(f"✅ Пользователь {uid} забанен на {minutes} минут", reply_markup=get_admin_keyboard())
    except:
        await m.answer("❌ Ошибка", reply_markup=get_admin_keyboard())
...
# ================= MAIN POLLING =================
async def main():
    logger.info("🚀 Starting Auron Search Bot (Polling mode on Render)...")
