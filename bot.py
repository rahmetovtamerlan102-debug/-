...
# -*- coding: utf-8 -*-
"""
Auron Search Bot - RENDER READY
С HTTP сервером для Health Check (чтобы не падал по таймауту)
"""
...
logger.info(f"✅ Bot started | Admin: {config.admin_id} | Port: {config.port}")
...
async def length_inline_kb(uid: int) -> InlineKeyboardMarkup:
    """Кнопка 5 видна всем, но с пометкой Premium"""
...
async def main():
    ...
    # Запускаем HTTP сервер для Health Check (чтобы Render не убивал бота)
    ...
    logger.info(f"🌐 HTTP Health Check server started on port {config.port}")
