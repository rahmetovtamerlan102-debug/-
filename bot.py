...
# -*- coding: utf-8 -*-
"""
Auron Search Bot - FULL PRODUCTION VERSION
С админ панелью (кнопки), поддержкой, баном, premium
"""
...
def get_main_keyboard(uid: int = None) -> ReplyKeyboardMarkup:
    """Главная клавиатура - если админ, показывает админ кнопку"""
    buttons = [
...
def get_admin_keyboard() -> ReplyKeyboardMarkup:
    ...
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="📊 Статистика"), KeyboardButton(text="💰 Заявки USDT")],
            [KeyboardButton(text="👥 Пользователи"), KeyboardButton(text="📨 Сообщения")],
            [KeyboardButton(text="🎁 Выдать Premium"), KeyboardButton(text="🚫 Забанить")],
...
async def handle_support_message(m: Message, state: FSMContext):
    ...
    await m.answer(
        ...
        "Ответ придет в личные сообщения.",
        parse_mode="Markdown",
        reply_markup=get_main_keyboard(m.from_user.id)
    )
    await state.clear()
...
# ================= АДМИН ПАНЕЛЬ (КНОПКИ, БЕЗ КОМАНД) =================
...
async def admin_payments(m: Message):
    ...
    if not rows:
        await m.answer("📭 Нет ожидающих заявок", reply_markup=get_admin_keyboard())
        return
    for p in rows:
        kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def admin_users(m: Message):
    ...
    if not rows:
        await m.answer("📭 Нет пользователей", reply_markup=get_admin_keyboard())
        return
    text = "👥 *Последние пользователи:*\n\n"
    for r in rows:
        prem = "💎" if is_premium(dict(r)) else "🔹"
        text += f"{prem} ID: `{r['user_id']}` | Поисков: {r['searches']} | Реф: {r['refs']}\n"
    await m.answer(text, parse_mode="Markdown", reply_markup=get_admin_keyboard())
...
async def admin_messages(m: Message):
    ...
    if not rows:
        await m.answer("📭 Нет новых сообщений", reply_markup=get_admin_keyboard())
        return
    for r in rows:
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton("✅ Пометить прочитанным", callback_data=f"msg_read_{r['id']}")]
        ])
        await m.answer(
            f"📨 *Сообщение #{r['id']}*\n"
            f"👤 ID: `{r['user_id']}`\n"
            f"📅 {time.strftime('%d.%m.%Y %H:%M', time.localtime(r['created_at']))}\n\n"
            f"💬 {r['text']}",
            parse_mode="Markdown",
            reply_markup=kb
        )

async def admin_give_premium_start(m: Message, state: FSMContext):
    if m.from_user.id != config.admin_id:
        return
    await state.set_state(AdminPremiumState.waiting_for_user_id)
    await m.answer(
        "🎁 *Выдача Premium*\n\n"
...
@dp.message(AdminPremiumState.waiting_for_user_id)
async def admin_premium_get_id(m: Message, state: FSMContext):
    if m.text == "🔙 Отмена":
        await state.clear()
        await m.answer("❌ Отменено", reply_markup=get_admin_keyboard())
        return
    try:
        ...
        await state.set_state(AdminPremiumState.waiting_for_days)
    except:
        await m.answer("❌ Неверный ID! Введите число.")

@dp.message(AdminPremiumState.waiting_for_days)
async def admin_premium_set_days(m: Message, state: FSMContext):
    ...
    try:
        ...
        days_text = "навсегда" if days == -1 else f"{days} дней" if days > 0 else "снят"
        await m.answer(
            f"✅ Premium {days_text} для пользователя `{user_id}`",
            parse_mode="Markdown",
            reply_markup=get_admin_keyboard()
        )
        try:
...
async def admin_ban_start(m: Message, state: FSMContext):
    if m.from_user.id != config.admin_id:
        return
...
async def admin_ban_get_id(m: Message, state: FSMContext):
    ...
    try:
        ...
        await state.set_state(AdminBanState.waiting_for_minutes)
    except:
        await m.answer("❌ Неверный ID! Введите число.")

@dp.message(AdminBanState.waiting_for_minutes)
async def admin_ban_set_minutes(m: Message, state: FSMContext):
    ...
    try:
        ...
        if minutes == 0:
            await app.rate_limiter.unban_user(user_id)
            await m.answer(
            ...
            try:
                await app.bot.send_message(user_id, "✅ *Ваш бан снят!*", parse_mode="Markdown")
            except:
                pass
        else:
            seconds = minutes * 60
            await app.rate_limiter.ban_user(user_id, seconds)
            await m.answer(
                f"✅ Пользователь `{user_id}` забанен на {minutes} минут",
                parse_mode="Markdown",
                reply_markup=get_admin_keyboard()
            )
            try:
                await app.bot.send_message(
                    user_id,
                    f"🚫 *Вы забанены на {minutes} минут!*\n\nПричина: нарушение правил",
                    parse_mode="Markdown"
                )
            except:
                pass
...
async def admin_back_to_main(m: Message):
    if m.from_user.id != config.admin_id:
        return
    await m.answer(
        "👋 *Главное меню*",
...
async def cb_approve(c: CallbackQuery):
    ...
    if pay:
        await set_premium(pay["user_id"], USDT_DAYS.get(pay["plan"], 30))
...
async def payment_success(m: Message):
    ...
    await m.answer("✅ *Premium активирован!*", parse_mode="Markdown", reply_markup=get_main_keyboard(m.from_user.id))

# ================= РЕГИСТРАЦИЯ ХЕНДЛЕРОВ =================
def register_handlers(dp: Dispatcher):
    # Основные
    dp.message.register(cmd_start, Command("start"))
    dp.message.register(cmd_find_username, F.text == "🔍 Найти юзернейм")
    dp.message.register(cmd_premium, F.text == "💎 Купить Premium")
    dp.message.register(cmd_profile, F.text == "👤 Личный кабинет")
    dp.message.register(cmd_support, F.text == "🆘 Поддержка")
    dp.message.register(cmd_my_usernames, Command("my_usernames"))
    
    # Админ панель (кнопки)
    dp.message.register(cmd_admin_panel, F.text == "🔧 Админ панель")
    dp.message.register(admin_stats, F.text == "📊 Статистика")
    dp.message.register(admin_payments, F.text == "💰 Заявки USDT")
    dp.message.register(admin_users, F.text == "👥 Пользователи")
    dp.message.register(admin_messages, F.text == "📨 Сообщения")
    dp.message.register(admin_give_premium_start, F.text == "🎁 Выдать Premium")
    dp.message.register(admin_ban_start, F.text == "🚫 Забанить")
    dp.message.register(admin_back_to_main, F.text == "🔙 В главное меню")
    
    # FSM для админа
    dp.message.register(admin_premium_get_id, AdminPremiumState.waiting_for_user_id)
    dp.message.register(admin_premium_set_days, AdminPremiumState.waiting_for_days)
    dp.message.register(admin_ban_get_id, AdminBanState.waiting_for_user_id)
    dp.message.register(admin_ban_set_minutes, AdminBanState.waiting_for_minutes)
    
    # Callbacks
    dp.callback_query.register(cb_back_main, F.data == "back_main")
    dp.callback_query.register(cb_back_to_length, F.data == "back_to_length")
    ...
    dp.callback_query.register(cb_msg_read, F.data.startswith("msg_read_"))
    ...
    dp.message.register(handle_support_message, SupportState.waiting_for_message)
    
    # Команды для админа (только для выдачи premium и бана, но они и так есть в кнопках)
    dp.message.register(cmd_premium_give, Command("premium"))

# ================= ФИКТИВНЫЕ КОМАНДЫ ДЛЯ АДМИНА (чтоб не падало) =================
async def cmd_premium_give(m: Message):
    if m.from_user.id != config.admin_id:
        return
    await m.answer("Используйте кнопку '🎁 Выдать Premium' в админ панели", reply_markup=get_admin_keyboard())

async def cmd_ban(m: Message):
    if m.from_user.id != config.admin_id:
        return
    await m.answer("Используйте кнопку '🚫 Забанить' в админ панели", reply_markup=get_admin_keyboard())

async def cmd_unban(m: Message):
    if m.from_user.id != config.admin_id:
        return
    await m.answer("Для снятия бана введите 0 минут через кнопку '🚫 Забанить'", reply_markup=get_admin_keyboard())
...
async def main():
    ...
    print("🟢 1. BEFORE DB INIT")
    await init_db()
    print("🟢 2. AFTER DB INIT")
    
    print("🟢 3. INITIALIZING DISPATCHER")
    ...
    print("🟢 4. DISPATCHER READY")
    
    print("🟢 5. CREATING BOT")
    app.bot = Bot(token=config.bot_token)
    asyncio.create_task(cleanup_task())
    print("🟢 6. BOT CREATED")
    
    print("🟢 7. BEFORE POLLING")
    logger.info("🤖 BOT IS NOW POLLING...")
    await app.dp.start_polling(app.bot)
    print("🟢 8. POLLING STARTED")

if __name__ == "__main__":
    try:
        print("🟢 STARTING BOT...")
        asyncio.run(main())
        print("🟢 BOT FINISHED (should not happen)")
    except Exception as e:
