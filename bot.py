...
# ================= КОНФИГУРАЦИЯ =================
TOKEN = "8705540280:AAFywYAvZIARQi5MXM9SpecMC7PMwnj9gPM"
ADMIN_ID = 8276815852
FREE_LIMIT = 10
CHANNEL_USERNAME = "@Auron_search"
USDT_WALLET = "#IV50644032"
BOT_USERNAME = "Auronsearchesbot"
...
if "referals_count" not in cols:
    c.execute("ALTER TABLE users ADD COLUMN referals_count INTEGER DEFAULT 0")
...
# ================= CALLBACK-ЗАПРОСЫ (ПОЛНАЯ ВЕРСИЯ) =================
...
def handle_callback(call):
    try:
        ...
        if data.startswith("len_"):
            ...
            is_premium, _, _, _, _ = get_user(uid)
            # Проверка для длины 5 – только Premium
        ...
        # Отдельная кнопка "Ещё" для длины 7
        if data == "gen_7":
            if not can_search(uid):
                remaining = get_remaining(uid)
                bot.answer_callback_query(call.id, f"❌ Лимит исчерпан! Осталось: {remaining}. Завтра обнулится.", show_alert=True)
        ...
        # Админ-панель: сообщения поддержки
        ...
        # Админ-панель: список заявок USDT
        ...
        if data.startswith("admin_usdt_approve_"):
            ...
            c.execute("SELECT user_id, days FROM pending_payments WHERE id=? AND status='waiting'", (payment_id,))
            row = c.fetchone()
            if not row:
            ...
            conn.commit()
            bot.send_message(user_id, f"🎉 *Premium активирован!* {'Навсегда' if days==0 else f'На {days} дней.'}", parse_mode="Markdown")
        ...
        if data.startswith("admin_usdt_reject_"):
            if uid != ADMIN_ID:
                ...
                return
            payment_id = int(data.split("_")[3])
            c.execute("SELECT user_id FROM pending_payments WHERE id=? AND status='waiting'", (payment_id,))
            row = c.fetchone()
            if not row:
                bot.answer_callback_query(call.id, "❌ Заявка уже обработана")
                return
            user_id = row[0]
            c.execute("UPDATE pending_payments SET status='cancelled' WHERE id=?", (payment_id,))
            conn.commit()
            bot.send_message(user_id, "❌ Ваша заявка на оплату USDT отклонена администратором.")
        ...
        if data == "admin_back_to_main":
            if uid != ADMIN_ID:
                bot.answer_callback_query(call.id, "⛔ Нет прав")
                return
            bot.edit_message_text("🔧 *Админ панель*", call.message.chat.id, call.message.message_id, reply_markup=admin_main_keyboard(), parse_mode="Markdown")
            bot.answer_callback_query(call.id)
            return

        # Прочие callback (например, выбор метода оплаты)
        if data == "back_to_payment_method":
            bot.edit_message_text("🌟 *Выберите способ оплаты Premium:*", call.message.chat.id, call.message.message_id, reply_markup=payment_method_keyboard(), parse_mode="Markdown")
            bot.answer_callback_query(call.id)
            return

        if data == "pay_method_stars":
            bot.edit_message_text("⭐ *Оплата через Telegram Stars*\n\nВыберите тариф:", call.message.chat.id, call.message.message_id, reply_markup=stars_tariffs_keyboard(), parse_mode="Markdown")
            bot.answer_callback_query(call.id)
            return

        if data == "pay_method_usdt":
            bot.edit_message_text("🪙 *Оплата через USDT (Crypto Bot)*\n\nВыберите тариф:", call.message.chat.id, call.message.message_id, reply_markup=usdt_tariffs_keyboard(), parse_mode="Markdown")
            bot.answer_callback_query(call.id)
            return
        ...
        if data.startswith("stars_"):
            ...
            bot.send_invoice(call.message.chat.id, title="Auron Premium", description=f"Premium на {desc}", invoice_payload=f"stars_{days}_{stars}_{uid}", provider_token="", currency="XTR", prices=[LabeledPrice(label="Premium", amount=stars)], start_parameter="auron_premium")
            bot.answer_callback_query(call.id)
            return
        ...
        if data.startswith("usdt_"):
            ...
            c.execute("INSERT INTO pending_payments (user_id, tariff, amount, days, created_at) VALUES (?, ?, ?, ?, ?)", (uid, tariff, amount, days, int(time.time())))
            conn.commit()
            pid = c.lastrowid
            text = (f"💸 *Счёт создан!*\nСумма: {amount} USDT\nТариф: {days if days!=0 else 'навсегда'} дней\n\n*Как оплатить:*\n1. Переведите {amount} USDT (TRC20) на кошелёк:\n`{USDT_WALLET}`\n2. Нажмите «✅ Я оплатил»\n3. Админ проверит платёж.")
            markup = InlineKeyboardMarkup()
            markup.add(InlineKeyboardButton("✅ Я оплатил", callback_data=f"confirm_usdt_{pid}"), InlineKeyboardButton("◀ Назад", callback_data="back_to_payment_method"))
            bot.edit_message_text(text, call.message.chat.id, call.message.message_id, reply_markup=markup, parse_mode="Markdown")
            bot.answer_callback_query(call.id)
            return
        ...
        if data.startswith("confirm_usdt_"):
            ...
            if not row:
                ...
                return
            admin_text = f"🆕 ОПЛАТА USDT\nПользователь: {row[0]}\nТариф: {row[3] if row[3]!=0 else 'навсегда'} дней\nСумма: {row[2]} USDT\nID: {pid}"
            markup = InlineKeyboardMarkup()
            markup.add(InlineKeyboardButton("✅ Подтвердить", callback_data=f"admin_approve_{pid}"), InlineKeyboardButton("❌ Отклонить", callback_data=f"admin_reject_{pid}"))
        ...
        if data.startswith("admin_approve_"):
            ...
            conn.commit()
            bot.send_message(row[0], f"🎉 *Premium активирован!* {'Навсегда' if row[1]==0 else f'На {row[1]} дней.'}", parse_mode="Markdown")
            bot.edit_message_text(f"✅ Заявка #{pid} подтверждена.", call.message.chat.id, call.message.message_id)
            bot.answer_callback_query(call.id)
            return
        ...
        if data.startswith("admin_reject_"):
            ...
            bot.edit_message_text(f"❌ Заявка #{pid} отклонена.", call.message.chat.id, call.message.message_id)
            bot.answer_callback_query(call.id)
            return
...
def handle_successful_payment(message):
    ...
    if len(parts) >= 3 and parts[0] == "stars":
        set_premium(message.from_user.id, int(parts[1]))
        bot.send_message(message.chat.id, f"✅ *Premium активирован* на {'навсегда' if int(parts[1])==0 else f'{parts[1]} дней'}!", parse_mode="Markdown", reply_markup=start_keyboard(message.from_user.id))
...
def cmd_premium(message):
    ...
    if len(args) < 2:
        bot.reply_to(message, "Использование: /premium <user_id> [дни] (0=навсегда)")
        return
    try:
        target = int(args[1])
        days = int(args[2]) if len(args) > 2 else 0
        set_premium(target, days)
        bot.reply_to(message, f"✅ Premium выдан {target} на {'навсегда' if days==0 else f'{days} дней'}.")
    except:
        bot.reply_to(message, "Ошибка в аргументах.")

@bot.message_handler(commands=["unpremium"])
def cmd_unpremium(message):
    if message.from_user.id != ADMIN_ID:
        return
    try:
        target = int(message.text.split()[1])
        c.execute("UPDATE users SET premium_until=0 WHERE user_id=?", (target,))
        conn.commit()
        bot.reply_to(message, f"✅ Premium снят с {target}.")
    except:
        bot.reply_to(message, "Ошибка. Использование: /unpremium <user_id>")

# ================= ЗАПУСК =================
if __name__ == "__main__":
    logger.info("🚀 Бот запущен. Реферальная система активна.")
    while True:
        try:
