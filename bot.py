...
async def menu_len(call: types.CallbackQuery):
    ...
    txt = "🔍 *Выберите длину username:*" if rem == float('inf') else f"🎫 *Осталось {rem} попыток.*\n\n🔍 Выберите длину:"
    kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def admin_panel(call: types.CallbackQuery):
    ...
    await call.message.edit_text("🔧 *АДМИН ПАНЕЛЬ*", parse_mode="Markdown", reply_markup=kb)
    await call.answer()
...
async def admin_stats(call: types.CallbackQuery):
    ...
    async with db.pool.acquire() as conn:
        ...
        premium = await conn.fetchval("SELECT COUNT(*) FROM users WHERE premium_until > $1 OR premium_until = $2", 
                                      int(time.time()), FOREVER_PREMIUM)
        today = date.today()
        searches = await conn.fetchval("SELECT COALESCE(SUM(searches_today), 0) FROM users WHERE last_date = $1", today)
    text = f"📊 *СТАТИСТИКА*\n\n👥 Пользователей: {users}\n👑 Premium: {premium}\n🔍 Поисков сегодня: {searches}"
...
async def admin_payments(call: types.CallbackQuery):
    ...
    if not payments:
        await call.message.edit_text("💰 *Нет ожидающих оплат*", parse_mode="Markdown")
        await call.answer()
        return
    text = "💰 *Ожидают оплаты:*\n\n"
    for p in payments:
        days_text = "навсегда" if p['days'] == 0 else f"{p['days']} дн"
        text += f"ID: {p['id']} | Пользователь: {p['user_id']} | Тариф: {days_text}\n"
    kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def admin_broadcast(call: types.CallbackQuery, state: FSMContext):
    if call.from_user.id != ADMIN_ID:
        await call.answer("Доступ запрещён", show_alert=True)
        return
    await call.message.edit_text("📢 *Введите текст рассылки:*", parse_mode="Markdown")
...
async def send_broadcast(message: types.Message, state: FSMContext):
    ...
    for user in users:
        try:
            ...
            pass
    await message.answer(f"✅ *Рассылка завершена!*\n\nОтправлено: {sent} пользователям", parse_mode="Markdown")
    await state.clear()
    await send_main_menu(message.chat.id, ADMIN_ID)
